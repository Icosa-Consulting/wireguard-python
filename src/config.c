...
/*
	...
	Portions modified to maintain portability. Icosa Consulting Inc (c) 2020
*/
...
static const char *get_value(const char *line, const char *key)
{
	...
	if (keylen >= linelen) return NULL;

	if (strncasecmp(line, key, keylen)) return NULL;
...
static inline bool parse_port(uint16_t *port, uint32_t *flags, const char *value)
{
	...
	struct addrinfo hints = { .ai_family = AF_UNSPEC, .ai_socktype = SOCK_DGRAM, .ai_protocol = IPPROTO_UDP, .ai_flags = AI_PASSIVE };

	if (!strlen(value))
	{
		sprintf(config_err_msg, "Unable to parse empty port");
		return false;
	}
	...
	if (ret)
	{
		sprintf(config_err_msg, "%s: `%s'", ret == EAI_SYSTEM ? strerror(errno) : gai_strerror(ret), value);
		return false;
	}
	...
	else if (resolved->ai_family == AF_INET6 && resolved->ai_addrlen == sizeof(struct sockaddr_in6))
	{
		*port = ntohs(((struct sockaddr_in6 *)resolved->ai_addr)->sin6_port);
		...
		sprintf(config_err_msg, "Neither IPv4 nor IPv6 address found: `%s'", value);

	freeaddrinfo(resolved);
	if (!ret) *flags |= WGDEVICE_HAS_LISTEN_PORT;
	return ret == 0;
}
...
static inline bool parse_fwmark(uint32_t *fwmark, uint32_t *flags, const char *value)
{
	...
	if (!char_is_digit(value[0])) goto err;

	if (strlen(value) > 2 && value[0] == '0' && value[1] == 'x') base = 16;

	ret = strtoul(value, &end, base);
	if (*end || ret > UINT32_MAX) goto err;

	*fwmark = ret;
	*flags |= WGDEVICE_HAS_FWMARK;
	return true;
err:
	sprintf(config_err_msg, "Fwmark is neither 0/off nor 0-0xffffffff: `%s'", value);
	return false;
}

static inline bool parse_key(uint8_t key[static WG_KEY_LEN], const char *value)
{
	if (wg_key_from_base64(key, value) != 0)
	{
		sprintf(config_err_msg, "Key is not the correct length or format: `%s'", value);
...
static bool parse_keyfile(uint8_t key[static WG_KEY_LEN], const char *path)
{
	...
	{
		/* If we're at the end and we didn't read anything, we're /dev/null or an
		 * empty file. */
		...
		sprintf(config_err_msg, "Invalid length key in key file");
		goto out;
	}
	...
	while ((c = getc(f)) != EOF)
	{
		if (!char_is_space(c))
		{
			sprintf(config_err_msg, "Found trailing character in key file: `%c'", c);
			goto out;
		}
	}
	if (ferror(f) && errno)
...
static inline bool parse_ip(struct wg_allowedip *allowedip, const char *value)
{
	allowedip->family = AF_UNSPEC;
	if (strchr(value, ':'))
	{
		if (inet_pton(AF_INET6, value, &allowedip->ip6) == 1) allowedip->family = AF_INET6;
	}
	else
	{
		if (inet_pton(AF_INET, value, &allowedip->ip4) == 1) allowedip->family = AF_INET;
	}
...
static inline int parse_dns_retries(void)
{
	...
	if (*end || ret > INT_MAX)
	{
		sprintf(config_err_msg, "Unable to parse WG_ENDPOINT_RESOLUTION_RETRIES: `%s'", retries);
		exit(1);
	}
	return (int)ret;
}
...
static inline bool parse_endpoint(struct sockaddr *endpoint, const char *value)
{
	...
	{
		free(mutable);
		sprintf(config_err_msg, "Unable to parse empty endpoint");
		return false;
	}
	if (mutable[0] == '[')
	{
		...
		{
			free(mutable);
			sprintf(config_err_msg, "Unable to find matching brace of endpoint: `%s'", value);
			return false;
		}
		*end++ = '\0';
		if (*end++ != ':' || !*end)
		{
			free(mutable);
			sprintf(config_err_msg, "Unable to find port of endpoint: `%s'", value);
			return false;
		}
	}
	else
...
#define min(a, b) ((a) < (b) ? (a) : (b))
	for (unsigned int timeout = 1000000;; timeout = min(20000000, timeout * 6 / 5))
	{
		...
		sprintf(config_err_msg, "%s: `%s'. Trying again in %.2f seconds...",
...
static inline bool parse_persistent_keepalive(uint16_t *interval, uint32_t *flags, const char *value)
{
	...
	if (*end || ret > 65535) goto err;

	*interval = (uint16_t)ret;
	*flags |= WGPEER_HAS_PERSISTENT_KEEPALIVE_INTERVAL;
	return true;
err:
	sprintf(config_err_msg, "Persistent keepalive interval is neither 0/off nor 1-65535: `%s'", value);
	return false;
}

static bool validate_netmask(struct wg_allowedip *allowedip)
{
	uint32_t *ip;
	int last;
	...
	for (int i = last; i >= 0; --i)
	{
		...
		if (allowedip->cidr >= 32 * (i + 1)) break;
		if (allowedip->cidr > 32 * i) mask >>= (allowedip->cidr - 32 * i);
		if (ntohl(ip[i]) & mask) return false;
	}

	return true;
}

static inline bool parse_allowedips(struct wg_peer *peer, struct wg_allowedip **last_allowedip, const char *value)
{
	struct wg_allowedip *allowedip = *last_allowedip, *new_allowedip;
	...
	while ((mask = strsep(&sep, ",")))
	{
		...
		if (mask)
		{
			if (!char_is_digit(mask[0])) goto err;
		...
		if (!validate_netmask(new_allowedip))
			sprintf(config_err_msg, "Warning: AllowedIP has nonzero host part: %s/%s", ip, mask);
...
err:
	free(new_allowedip);
	free(mutable);
	sprintf(config_err_msg, "AllowedIP is not in the correct format: `%s'", saved_entry);
	free(saved_entry);
	return false;
}
...
static bool process_line(struct config_ctx *ctx, const char *line)
{
	...
	{
		struct wg_peer *new_peer = calloc(1, sizeof(struct wg_peer));
...
#define key_match(key) (value = get_value(line, key "="))
	...
	if (ctx->is_device_section)
	{
		...
		else if (key_match("PrivateKey"))
		{
			...
			if (ret) ctx->device->flags |= WGDEVICE_HAS_PRIVATE_KEY;
		}
		else
			goto error;
	}
	else if (ctx->is_peer_section)
	{
		...
		else if (key_match("PublicKey"))
		{
			...
			if (ret) ctx->last_peer->flags |= WGPEER_HAS_PUBLIC_KEY;
		}
		...
		else if (key_match("PresharedKey"))
		{
			...
			if (ret) ctx->last_peer->flags |= WGPEER_HAS_PRESHARED_KEY;
		}
		else
			goto error;
	}
...
error:
	sprintf(config_err_msg, "Line unrecognized: `%s'", line);
	return false;
}
...
bool config_read_line(struct config_ctx *ctx, const char *input)
{
	...
	{
		if (!char_is_space(input[i])) line[cleaned_len++] = input[i];
	}
	if (!cleaned_len) goto out;
	ret = process_line(ctx, line);
out:
	free(line);
	if (!ret) wg_free_device(ctx->device);
	return ret;
}
...
struct wg_device *config_read_finish(struct config_ctx *ctx)
{
	...
	for_each_wgpeer(ctx->device, peer)
	{
		...
		{
			sprintf(config_err_msg, "A peer is missing a public key");
			goto err;
		}
	}
	return ctx->device;
...
char *strip_spaces(const char *in)
{
	...
	for (i = 0, l = 0; i < t; ++i)
	{
		if (!char_is_space(in[i])) out[l++] = in[i];
	}
	return out;
}
...
{
	struct wg_device *device = calloc(1, sizeof(*device));
	struct wg_peer *peer = NULL;
	struct wg_allowedip *allowedip = NULL;
	config_err_msg = calloc(1, sizeof(*config_err_msg));

	if (!device || (config_err_msg == NULL))
	{
	...
	while (argc > 0)
	{
		...
		{
			if (!parse_port(&device->listen_port, &device->flags, argv[1])) goto error;
			argv += 2;
			argc -= 2;
		}
		else if (!strcmp(argv[0], "fwmark") && argc >= 2 && !peer)
		{
			if (!parse_fwmark(&device->fwmark, &device->flags, argv[1])) goto error;
			argv += 2;
			argc -= 2;
		}
		else if (!strcmp(argv[0], "private-key") && argc >= 2 && !peer)
		{
			if (!parse_keyfile(device->private_key, argv[1])) goto error;
		...
		{
			struct wg_peer *new_peer = calloc(1, sizeof(*new_peer));
			...
			peer = new_peer;
			if (!parse_key(peer->public_key, argv[1])) goto error;
		...
		{
			if (!parse_endpoint(&peer->endpoint.addr, argv[1])) goto error;
			argv += 2;
			argc -= 2;
		}
		...
		{
			if (!parse_persistent_keepalive(&peer->persistent_keepalive_interval, &peer->flags, argv[1])) goto error;
			argv += 2;
			argc -= 2;
		}
		else if (!strcmp(argv[0], "preshared-key") && argc >= 2 && peer)
		{
			if (!parse_keyfile(peer->preshared_key, argv[1])) goto error;
		...
		{
			sprintf(config_err_msg, "Invalid argument: %s", argv[0]);
			goto error;
		}
	}
	return device;
